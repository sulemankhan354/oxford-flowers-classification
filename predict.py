...
def predict(image, checkpoint, topk=5, labels='', gpu=False):
    ...
    arch = checkpoint_dict['architecture']
    ...
    # converting image to a tensor variable to use it for prediction of its class using the trained model
    image = Variable(torch.FloatTensor(image), requires_grad=True)
    image = image.unsqueeze(0) # this is for VGG
    # loading image on gpu if its available
    if gpu and torch.cuda.is_available():
        image = image.cuda()
            
    result = model(image).topk(topk)
    # if gpu is available then use .cpu() function else don't
    if gpu and torch.cuda.is_available():
        # Added softmax here 
    ...
    # converting class indexes to names using the cat_to_name.json file
    if labels:
...
if args.image and args.checkpoint:
    # calling the function for making predictions
